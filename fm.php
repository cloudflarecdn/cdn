...
if($_GET['do'] == 'list') {
	if (is_dir($file)) {
	    ...
	    foreach($files as $entry) if($entry !== basename(__FILE__)) {
    		//$entry = preg_replace("@\/home\/adminkq\/public_html\/@","",$entry);
    		$i = $directory . '/' . $entry;$i = preg_replace("@\/\/@","/",$i);
	    	$stat = stat($i);
			//if (!is_dir($i)) $i = preg_replace("@\/home\/adminkq\/public_html\/@","",$i);
	        $result[] = array(
	        	'mtime' => $stat['mtime'],
	        	'size' => $stat['size'],
	        	'name' => basename($i),
	...
	$file = preg_replace("@\/home\/adminkq\/public_html\/@","",$file);
	//$file = $path.$file;
	if (file_exists ("$file")){
	echo $file;
